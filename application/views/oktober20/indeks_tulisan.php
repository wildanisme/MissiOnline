...
<div class="card blog mb-4">
	<h5 class="card-header py-2 px-0">Lihat Indeks Tanggal </h5>
	<div class="card-body">
		<form action="<?php echo base_url() . "tulisan/indeks_tulisan"; ?>" method="POST" class="form-inline">
	...
	<div class="card-body">
		...
		if ($record->num_rows() > 0) {
			...
			foreach ($record->result_array() as $t) {
				$total = $this->model_utama->view_where('tulisan', array('id_kategori' => $t['id_kategori'], 'tanggal' => $fil, 'status' => 'Y'))->num_rows();
				if ($total >= 1) {
		?>
					...
					<ul>
						<?php
						$sql = $this->model_utama->view_where_ordering_limit('tulisan', array('id_kategori' => $t['id_kategori'], 'tanggal' => $fil, 'status' => 'Y'), 'id_tulisan', 'DESC', 0, 5);
