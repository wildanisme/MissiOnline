<?php
$baca = $rows['dibaca'] + 1;
$total_komentar = $this->model_utama->view_where('komentar', array('id_tulisan' => $rows['id_tulisan']))->num_rows();
?>
			...
			<div class="block">
					...
					<div class="shortcode-content">
						<div class="paragraph-row">
							...
							<div class="column12">
								<?php
								if ($rows['gambar'] != '') {
									echo "<img style='width:100%' src='" . base_url() . "asset/foto_tulisan/$rows[gambar]' alt='$rows[judul]' /></a><br><br>";
								}
								if ($rows['keterangan_gambar'] != '') {
									echo "<center><p><i><b>Keterangan Gambar :</b> $rows[keterangan_gambar]</i></p></center><br>";
								}

								$paragraph = explode("</p>", $rows['isi_tulisan']);
								if (empty($paragraph[3])) {
									...
									$cari = "SELECT * FROM tulisan WHERE (id_tulisan<'$ambil_id') and (id_tulisan!='$ambil_id') and (";
									...
									$cari .= ") ORDER BY id_tulisan DESC LIMIT 5";
									$hasil  = $this->db->query($cari);
									foreach ($hasil->result_array() as $row) {
										$total_komentar_terkait = $this->model_utama->view_where('komentar', array('id_tulisan' => $row['id_tulisan']))->num_rows();
				...
				<?php if ($total_komentar >= '1') { ?>
					<div id="viewcomment listcomment" class="block-title">
						...
						<h2><?php echo "Ada $total_komentar Komentar untuk tulisan Ini"; ?></h2>
					</div>
					<div class="block-content">
						<div class="comment-block">
							<ol class="comments">
								<li>
									<?php
									$no = 1;
									$komentar = $this->model_utama->view_where_ordering_limit('komentar', array('id_tulisan' => $rows['id_tulisan'], 'aktif' => 'Y'), 'id_komentar', 'ASC', 0, 100);
				...
				<div class="block-content">
					<div id="writecomment">
						<form action="<?php echo base_url(); ?>tulisan/kirim_komentar" method="POST" id="form_komentar">
							<input type="hidden" name='a' value='<?php echo "$rows[id_tulisan]"; ?>'>
