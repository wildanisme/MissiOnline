...
<head>
	...
	<?php if ($this->uri->segment(1) == 'tulisan' and $this->uri->segment(2) == 'detail') {
		$rows = $this->model_utama->view_where('tulisan', array('judul_seo' => $this->uri->segment(3)))->row_array();
		echo '<meta property="og:title" content="' . $title . '" />
			 <meta property="og:type" content="article" />
			 <meta property="og:url" content="' . base_url() . '' . $this->uri->segment(3) . '" />
			 <meta property="og:image" content="' . base_url() . 'asset/foto_tulisan/' . $rows['gambar'] . '" />
		...
		<div class="content">
			<div class="wrapper">
				<div class="breaking-news">
					...
					<ul>
						<?php
						$terkini = $this->model_utama->view_where_ordering_limit('tulisan', array('status' => 'Y'), 'id_tulisan', 'DESC', 0, 10);
				...
				if ($this->uri->segment(1) == 'kategori') {
					...
					if ($bb['gambar_utama'] != '') {
						if (preg_match("/swf\z/i", $string)) {
							echo "<embed width='100%' src='" . base_url() . "asset/foto_tulisan/$bb[gambar_utama]' quality='high' type='application/x-shockwave-flash'>";
						} else {
							echo "<a href='#' target='_blank'><img style='margin-bottom:5px' width='100%' src='" . base_url() . "asset/foto_tulisan/$bb[gambar_utama]' alt='$bb[nama_kategori]' /></a>";
						}
					}
				}
				?>
