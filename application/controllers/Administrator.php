...
class Administrator extends CI_Controller
{
    ...
    // Controller Modul List tulisan

    function listtulisan()
    {
        cek_session_akses('listtulisan', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('tulisan', 'id_tulisan', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('tulisan', array('username' => $this->session->username), 'id_tulisan', 'DESC');
        }
        $data['rss'] = $this->model_utama->view_joinn('tulisan', 'users', 'kategori', 'username', 'id_kategori', 'id_tulisan', 'DESC', 0, 10);
        $data['iden'] = $this->model_utama->view_where('identitas', array('id_identitas' => 1))->row_array();
        $this->load->view('administrator/rss', $data);
        $this->template->load('administrator/template', 'administrator/mod_tulisan/view_tulisan', $data);
    }

    function tambah_listtulisan()
    {
        cek_session_akses('listtulisan', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_tulisan/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '5000'; // kb
            $this->load->library('upload', $config);
            ...
            $config['source_image'] = 'asset/foto_tulisan/' . $hasil['file_name'];
            $config['wm_text'] = 'MISSI ONLINE';
            $config['wm_type'] = 'text';
            ...
            $this->model_app->insert('tulisan', $data);
            redirect($this->uri->segment(1) . '/listtulisan');
        } else {
            $data['tag'] = $this->model_app->view_ordering('tag', 'id_tag', 'DESC');
            $data['record'] = $this->model_app->view_ordering('kategori', 'id_kategori', 'DESC');
            $this->template->load('administrator/template', 'administrator/mod_tulisan/view_tulisan_tambah', $data);
        }
    }

    function edit_listtulisan()
    {
        cek_session_akses('listtulisan', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_tulisan' => $this->input->post('id'));
            $this->model_app->update('tulisan', $data, $where);
            redirect($this->uri->segment(1) . '/listtulisan');
        } else {
            $tag = $this->model_app->view_ordering('tag', 'id_tag', 'DESC');
            $record = $this->model_app->view_ordering('kategori', 'id_kategori', 'DESC');
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('tulisan', array('id_tulisan' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('tulisan', array('id_tulisan' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses, 'tag' => $tag, 'record' => $record);
            $this->template->load('administrator/template', 'administrator/mod_tulisan/view_tulisan_edit', $data);
        }
    }

    function publish_listtulisan()
    {
        cek_session_admin();
        ...
        $where = array('id_tulisan' => $this->uri->segment(3));
        $this->model_app->update('tulisan', $data, $where);
        redirect($this->uri->segment(1) . '/listtulisan');
    }

    function delete_listtulisan()
    {
        cek_session_akses('listtulisan', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_tulisan' => $this->uri->segment(3));
        } else {
            $id = array('id_tulisan' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('tulisan', $id);
        redirect($this->uri->segment(1) . '/listtulisan');
    }
    ...
    // Controller Modul Kategori tulisan

    function kategoritulisan()
    {
        cek_session_akses('kategoritulisan', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('kategori', 'id_kategori', 'DESC');
        } else {
    ...
    function tambah_kategoritulisan()
    {
        cek_session_akses('kategoritulisan', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_tulisan/';
    ...
    function delete_kategoritulisan()
    {
        cek_session_akses('kategoritulisan', $this->session->id_session);
        if ($this->session->level == 'admin') {
    ...
    // Controller Modul Tag tulisan

    function tagtulisan()
    {
        cek_session_akses('tagtulisan', $this->session->id_session);
        if ($this->session->level == 'admin') {
    ...
    // Controller Modul Komentar tulisan

    function komentartulisan()
    {
        cek_session_akses('komentartulisan', $this->session->id_session);
    ...
    function edit_komentartulisan()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            redirect($this->uri->segment(1) . '/komentartulisan');
        } else {
    ...
    function delete_komentartulisan()
    {
        cek_session_akses('komentartulisan', $this->session->id_session);
    ...
    // Controller Modul Sensor Komentar tulisan
    ...
    function edit_sensorkomentar()
    {
        ...
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $this->model_tulisan->tag_tulisan_update();
            $data = array(
                'kata' => $this->input->post('a'),
                'username' => $this->session->username,
                'ganti' => $this->input->post('b')
