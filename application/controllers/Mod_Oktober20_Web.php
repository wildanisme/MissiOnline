...
class Mod_Oktober20_Web extends CI_Controller
{
	...
	public function contact()
	{
		...
		if (isset($_POST['submit'])) {
			if (
				!empty($this->input->post('security_code')) &&
	...
	public function komentar_tulisan()
	{

		if (isset($_POST['submit'])) {

			$cek = $this->model_utama->view_where('tulisan', array('id_tulisan' => $this->input->post('tulisan')));
			$row = $cek->row_array();
			...
			if ($cek->num_rows() <= 0) {
				...
				if (
					...
					$data = array(
						'id_tulisan'	=>	cetak($this->input->post('tulisan', TRUE)),
			...
			redirect('tulisan/detail/' . $row['judul_seo'] . '#writecomment');
		}
	}
